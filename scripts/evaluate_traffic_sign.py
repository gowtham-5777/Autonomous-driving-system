"""Evaluate a trained traffic-sign YOLOv8 model on the validation split."""
...
from training.traffic_sign.gtsrb_converter import YOLO_CLASS_NAMES, YOLO_TO_ADAS_LABEL
...
def parse_args() -> argparse.Namespace:
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate traffic-sign YOLOv8 weights on the val split.",
    )
    parser.add_argument(
        "--weights",
        type=Path,
        default=defaults.model_output,
        help="Path to traffic_signs_yolov8n.pt.",
    )
    parser.add_argument(
        "--data",
        type=Path,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=_PROJECT_ROOT / "runs" / "traffic_sign" / "evaluation_metrics.json",
