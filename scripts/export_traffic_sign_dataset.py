"""Export the prepared traffic-sign YOLO dataset as a Colab-uploadable zip."""
...
from training.traffic_sign.gtsrb_converter import YOLO_CLASS_NAMES, write_dataset_yaml
from training.traffic_sign.config import TrafficSignTrainingConfig

logger = logging.getLogger(__name__)

DEFAULT_COLAB_DATASET_PATH = "/content/traffic_sign_yolo"
...
def _validate_dataset(dataset_dir: Path) -> dict[str, int]:
    ...
    if not yaml_path.is_file():
        raise FileNotFoundError(
            ...
            "Run scripts/prepare_traffic_sign_dataset.py first."
        )
    return counts
...
def export_dataset_zip(
    ...
    staging_yaml = output_zip.parent / "_colab_sign_dataset.yaml"
    ...
    file_entries.insert(0, (staging_yaml, "traffic_sign_yolo/dataset.yaml"))
...
def parse_args() -> argparse.Namespace:
    defaults = TrafficSignTrainingConfig()
    parser = argparse.ArgumentParser(
        description="Export traffic_sign_yolo dataset as a zip for Google Colab.",
    )
    parser.add_argument(
        "--dataset",
        type=Path,
        default=defaults.dataset_dir,
        help="Path to prepared traffic_sign_yolo directory.",
    )
    parser.add_argument(
        "--output",
        type=Path,
        default=_PROJECT_ROOT / "data" / "traffic_sign_yolo_colab.zip",
...
def main() -> int:
    ...
    print("\nUpload the zip to Google Drive, then follow docs/traffic_sign_colab_training.md")
    return 0
