"""Train YOLOv8n on the GTSRB-derived traffic-sign dataset."""
...
def train_model(config: TrafficSignTrainingConfig, resume: bool = False) -> Path:
    ...
    train_kwargs = {
        ...
        "seed": config.seed,
        "project": str(_PROJECT_ROOT / "runs" / "traffic_sign"),
        "name": "yolov8n_gtsrb",
        "exist_ok": True,
        "hsv_h": config.hsv_h,
        "hsv_s": config.hsv_s,
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train YOLOv8n traffic-sign detector.")
    parser.add_argument(
        "--data",
        type=Path,
        default=TrafficSignTrainingConfig().dataset_yaml,
    ...
    parser.add_argument(
        "--output",
        type=Path,
        default=TrafficSignTrainingConfig().model_output,
        help="Destination for traffic_signs_yolov8n.pt.",
    )
