#!/usr/bin/env python3
"""Gate script: verify traffic sign detection pipeline wiring and outputs.
...
Pass ``--real`` to run with real YOLOv8 sign weights when available.
"""
    ...
    class _StubEngine(YOLOv8SignInferenceEngine):
        ...
        def run(self, frame: np.ndarray) -> dict:
            ...
            return {
                ...
                "confidences": np.array([0.93], dtype=np.float32),
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Verify traffic sign detection pipeline")
    parser.add_argument(
        "--real",
        action="store_true",
        help="Use real fine-tuned YOLOv8 sign weights from config",
    )
    ...
    from src.utils.model_paths import get_traffic_sign_weights_path

    module_path = Path(inspect.getfile(TrafficSignModule))
    if not module_path.is_file():
        _fail(f"TrafficSignModule not found at {module_path}")
    _pass(f"TrafficSignModule at {module_path}")

    yolov8_sign_pkg = PROJECT_ROOT / "src" / "modules" / "yolov8_sign"
    for name in (
        "__init__.py",
        "model_loader.py",
        "inference.py",
        ...
        path = yolov8_sign_pkg / name
        if not path.is_file():
            _fail(f"missing {path}")
    _pass("yolov8_sign package files present")

    frame = np.zeros((720, 1280, 3), dtype=np.uint8)
    cv2 = __import__("cv2")
    cv2.rectangle(frame, (580, 120), (700, 240), (0, 0, 200), -1)
    ...
    if args.real:
        weights_path = get_traffic_sign_weights_path()
        if not weights_path.is_file():
            _fail(
                f"fine-tuned sign weights not found at {weights_path} "
                "(train or place traffic_signs_yolov8n.pt)"
            )
        ...
        module = TrafficSignModule(device="cpu", confidence_threshold=0.25)
        mode = "real"
    else:
    ...
    if not isinstance(result, TrafficSignDetectionResult):
        _fail(f"predict() returned {type(result)}, expected TrafficSignDetectionResult")
    _pass("inference runs")

    if result.summary.total_count < 1:
        if args.real and result.raw_status in {"ok", "verify_stub", "stub"}:
            print(
                "WARN: real inference returned 0 signs on test frame "
                "(expected without signs in scene)"
            )
            _pass("real inference completed (0 signs on test scene)")
        else:
    ...
    print()
    print("=" * 60)
    print("TRAFFIC SIGN DETECTION GATE: ALL CHECKS PASSED")
    print(f"  mode            = {mode}")
    ...
    print(f"  active_limit    = {result.summary.active_speed_limit_kmh}")
