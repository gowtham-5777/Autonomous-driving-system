"""YOLOv8 inference wrapper — forward-pass execution layer."""
...
logger = logging.getLogger("adas.modules.yolov8.inference")
...
class InferenceExecutionError(RuntimeError):
    """Raised when the YOLOv8 forward pass fails unexpectedly."""
...
class YOLOv8InferenceEngine:
    """Execute YOLOv8 object detection on video frames."""

    def __init__(
        self,
        model_package: ModelPackage | None = None,
        config: YOLOv8InferenceConfig | None = None,
    ) -> None:
        self.config = config or YOLOv8InferenceConfig()
        ...
        logger.info(
            "YOLOv8InferenceEngine created — ready=%s, imgsz=%d",
            self.is_ready,
            self.config.imgsz,
        )
    ...
    def attach_model(self, model_package: ModelPackage) -> None:
        """Attach a loaded YOLOv8 model package."""
        required_keys = {"model", "metadata"}
        missing = required_keys - set(model_package.keys())
        if missing:
            raise ValueError(
                f"Invalid YOLOv8 model package — missing keys: {sorted(missing)}"
            )

        model = model_package.get("model")
        if model is None:
            raise ValueError("Invalid YOLOv8 model package — model is None")

        self.model_package = model_package
        self._model = model
        logger.info("YOLOv8 model attached to inference engine")

    def detach_model(self) -> None:
        """Detach the current model package."""
        logger.info("Detaching YOLOv8 model from inference engine")
        self.model_package = None
        self._model = None

    def run(self, frame: Frame) -> YOLOv8RawOutput:
        """Run YOLOv8 detection on a single BGR frame."""
        if not self.is_ready:
            raise InferenceNotReadyError(
                "YOLOv8 inference engine is not ready — attach a model package first"
            )

        self._validate_frame(frame)
        logger.info("Running YOLOv8 inference on frame shape %s", frame.shape)
        ...
        try:
            ...
            logger.exception("YOLOv8 forward pass failed")
            raise InferenceExecutionError(f"YOLOv8 forward pass failed: {exc}") from exc
