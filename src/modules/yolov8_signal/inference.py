"""YOLOv8 signal inference wrapper — forward-pass execution layer."""
...
logger = logging.getLogger("adas.modules.yolov8_signal.inference")
...
class InferenceExecutionError(RuntimeError):
    """Raised when the YOLOv8 signal forward pass fails unexpectedly."""


@dataclass(frozen=True)
class YOLOv8SignalInferenceConfig:
    """Configuration for YOLOv8 signal inference."""
...
class YOLOv8SignalInferenceEngine:
    """Execute YOLOv8 traffic signal detection on video frames."""

    def __init__(
        self,
        model_package: ModelPackage | None = None,
        config: YOLOv8SignalInferenceConfig | None = None,
    ) -> None:
        self.config = config or YOLOv8SignalInferenceConfig()
        ...
        logger.info(
            "YOLOv8SignalInferenceEngine created — ready=%s, imgsz=%d",
            self.is_ready,
            self.config.imgsz,
        )
    ...
    def attach_model(self, model_package: ModelPackage) -> None:
        """Attach a loaded YOLOv8 signal model package."""
        required_keys = {"model", "metadata"}
        missing = required_keys - set(model_package.keys())
        if missing:
            raise ValueError(
                f"Invalid YOLOv8 signal model package — missing keys: {sorted(missing)}"
            )

        model = model_package.get("model")
        if model is None:
            raise ValueError("Invalid YOLOv8 signal model package — model is None")

        self.model_package = model_package
        self._model = model
        logger.info("YOLOv8 signal model attached to inference engine")

    def detach_model(self) -> None:
        """Detach the current model package."""
        logger.info("Detaching YOLOv8 signal model from inference engine")
        self.model_package = None
        self._model = None

    def run(self, frame: Frame) -> YOLOv8SignalRawOutput:
        """Run YOLOv8 signal detection on a single BGR frame."""
        if not self.is_ready:
            raise InferenceNotReadyError(
                "YOLOv8 signal inference engine is not ready — attach a model package first"
            )

        self._validate_frame(frame)
        logger.info("Running YOLOv8 signal inference on frame shape %s", frame.shape)
        ...
        try:
            ...
            logger.exception("YOLOv8 signal forward pass failed")
            raise InferenceExecutionError(
                f"YOLOv8 signal forward pass failed: {exc}"
            ) from exc
