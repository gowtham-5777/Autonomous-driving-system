"""YOLOv8 signal model loading and weight management."""
...
logger = logging.getLogger("adas.modules.yolov8_signal.model_loader")
...
class WeightsNotFoundError(FileNotFoundError):
    """Raised when fine-tuned signal weights cannot be resolved."""
...
class WeightsMetadata:
    """Metadata describing a successfully loaded YOLOv8 signal model."""
...
def resolve_variant_name(variant: str) -> str:
    ...
    if normalized not in ALLOWED_VARIANTS:
        raise WeightsValidationError(
            f"Invalid YOLOv8 signal variant '{variant}'. "
            f"Expected one of: {sorted(ALLOWED_VARIANTS)}"
        )
    return normalized


class YOLOv8SignalModelLoader:
    """Load and manage fine-tuned YOLOv8 signal checkpoints via Ultralytics.

    Unlike the vehicle detector, there is **no** public COCO fallback —
    fine-tuned signal weights must exist at the configured path.
    """
    ...
    def __init__(
        ...
        logger.info(
            "YOLOv8SignalModelLoader created — variant=yolov8%s, device=%s",
            self.model_variant,
            device,
        )
    ...
    def resolve_weights_source(self) -> str:
        """Resolve the weight source path for fine-tuned signal weights."""
        if self.weights_path is None:
            raise WeightsNotFoundError(
                "Traffic signal weights path is not configured. "
        ...
        if self.weights_path.exists() and not self.weights_path.is_file():
            raise WeightsValidationError(
                f"YOLOv8 signal weights path is not a file: {self.weights_path}"
            )

        raise WeightsNotFoundError(
            f"Fine-tuned traffic signal weights not found at {self.weights_path}. "
            "Train or place traffic_signals_yolov8n.pt before running real inference."
        )

    def load_model(self) -> WeightsMetadata:
        """Load fine-tuned YOLOv8 signal weights via Ultralytics."""
        try:
            from ultralytics import YOLO
        except ImportError as exc:
            raise WeightsLoadError(
                "ultralytics is required for YOLOv8 traffic signal detection. "
        ...
        logger.info("Loading YOLOv8 signal model from %s", source)

        try:
            self._model = YOLO(source)
        except Exception as exc:
            logger.exception("YOLOv8 signal model load failed")
            raise WeightsLoadError(
                f"Failed to load YOLOv8 signal model from {source}: {exc}"
            ) from exc
        ...
        logger.info(
            "YOLOv8 signal model loaded — source=%s, variant=yolov8%s",
            source,
            self.model_variant,
        )
    ...
    def get_model(self) -> dict[str, Any]:
        ...
        if self._model is None or self._metadata is None:
            raise RuntimeError(
                "YOLOv8 signal model is not loaded. Call load_model() first."
            )
    ...
    def unload(self) -> None:
        ...
        logger.info("Unloading YOLOv8 signal model")
        self._model = None
        self._metadata = None
